...
int main(){
  ...
  in->numline=0;
  int cap=10,count=0;
  in->pointerArr=(char**)malloc(sizeof(char *)*cap);//mallocat for pointerArr, an array contains 10 pointers
  ...
  do{
    ...
    if(count>cap-1){
      cap=cap*2;
      in=(struct inputText *)realloc(in,cap*sizeof(struct inputText));//realloc strcut
	...
	printf("Success to realloc,cap is %d\n",cap);
      ...
      in->pointerArr=in->pointerArr+count-1;//+in->numline;
      printf("Now is element:%s \n ",*in->pointerArr);
      //printf("%s\n",*(in->pointerArr+1));
  ...
  char **newArr;
  newArr=(char **)malloc(cap*sizeof(char *));
  if(newArr== NULL){
    puts("Fail");
    return 1;
  }
