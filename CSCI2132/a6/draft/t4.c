...
int main(){
  ...
  do{
    ...
    if(count>cap-1){
      ...
      cap=cap*2;
      printf("cap is %d\n",cap);
      struct inputText **text= malloc(sizeof(struct inputText)*cap);
      text=malloc(sizeof(char *)*cap);
      int i=0;
      in->pointerArr=in->pointerArr-in->numline+1;
      //memcpy(text,in->pointerArr,sizeof(in->pointerArr));
      //text=in->pointerArr;
      /*  
      for(i;i<in->numline;i++){
	memcpy(text,*in->pointerArr,sizeof(in->pointerArr));
	text++;
	in->pointerArr++;
	}
